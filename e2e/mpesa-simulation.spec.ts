...
test.afterAll(() => {
  ...
  const report = {
    scenario: "M-Pesa callback simulation test (dev endpoint)",
    baseUrl: BASE_URL,
  ...
  const file = resolve(resultsDir, `simulation-test-${stamp}.json`);
  writeFileSync(file, JSON.stringify(report, null, 2));

  console.log("\n=== RETAILTROVE E2E SIMULATION TEST ===");
...
test("Full purchase simulation → wishlist → cart ×6 → M-Pesa sandbox → paid + order_items in Supabase", async ({
  ...
  // (fetchPurchaseFindings not available in helpers/db, skip for now)
