...
type ProductFormData = Omit<
  VendorProduct,
  "id" | "rating" | "vendorId" | "approvalStatus" | "createdAt"
>;
...
function handleInput(setter: (v: ProductFormData) => void, current: ProductFormData) {
    ...
    <div className="grid gap-4 py-2">
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          <Label>Name</Label>
          <Input name="name" value={data.name} onChange={handleInput(setData, data)} />
        </div>
        <div className="space-y-1">
      ...
      <div className="space-y-1">
        ...
        <Input name="description" value={data.description} onChange={handleInput(setData, data)} />
      </div>
      <div className="space-y-1">
        <Label>Image URL</Label>
        <Input name="imageUrl" value={data.imageUrl} onChange={handleInput(setData, data)} />
      </div>
      <Separator />
      <div className="grid grid-cols-3 gap-4">
        <div className="space-y-1">
          <Label>Price</Label>
          <Input name="price" value={data.price} onChange={handleInput(setData, data)} />
        </div>
        <div className="space-y-1">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <Select
            value={String(data.inStock ?? true)}
            onValueChange={(v) => setData({ ...data, inStock: v === "true" })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
...
export default function VendorPage() {
  useEffect(() => {
    document.title = "Vendor Dashboard - RetailTrove";
  }, []);
  const { user } = useAuth();
  ...
  const [editingFaq, setEditingFaq] = useState<VendorFaq | null>(null);
  ...
  // ── Derived ──────────────────────────────────────────────────────────────────
  const approvedProducts = myProducts.filter((p) => p.approvalStatus === "approved");
  const pendingProducts = myProducts.filter((p) => p.approvalStatus === "pending");
  const rejectedProducts = myProducts.filter((p) => p.approvalStatus === "rejected");
  ...
  const updateFaqMutation = useMutation({
    mutationFn: (data: VendorFaq) => apiRequest("PUT", `/api/faqs/${data.id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/faqs/mine"] });
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center px-4">
        ...
        <p className="text-gray-500 max-w-sm">
          Your vendor account is awaiting approval from an admin. You'll receive access to your
    ...
    <div className="container py-6 max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            ...
            <span className="ml-2 inline-block text-[10px] font-bold uppercase px-2 py-0.5 rounded-full text-white bg-emerald-600">
              vendor
            </span>
          </p>
        </div>
        ...
        <CardContent className="pt-0">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
                ...
                <Table>
                  ...
                  <TableBody>
                    {productsLoading ? (
                      ...
                      filteredProducts.map((p: VendorProduct) => (
                        <TableRow key={p.id}>
                          ...
                          <TableCell className="text-center">
                            <Badge
                              variant={
                                statusColor(p.approvalStatus) as
            ...
            <TabsContent value="new">
              <div className="max-w-2xl">
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  <Button variant="outline" onClick={() => setNewProduct({ ...EMPTY_PRODUCT })}>
                    Reset
                  </Button>
                </div>
              </div>
            ...
            <TabsContent value="faqs">
              <div className="flex justify-between items-center mb-4">
                <p className="text-sm text-muted-foreground">{myFaqs.length} submissions</p>
                        ...
                        <div className="flex justify-between items-start gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <Badge
                                variant={
                                  statusColor(f.status) as "default" | "secondary" | "destructive"
                                }
                                className="text-xs capitalize"
                              >
                                {f.status}
            ...
            <TabsContent value="customers">
              <p className="text-sm text-muted-foreground mb-4">
                {customers.length} registered customers (read-only view)
              </p>
              <div className="border rounded-md overflow-x-auto">
                <Table>
                  <TableHeader>
      ...
      <Dialog open={isAddFaqOpen} onOpenChange={setIsAddFaqOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Submit FAQ</DialogTitle>
            <DialogDescription>
              Your FAQ will be reviewed by an admin before publishing.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-2">
            <div className="space-y-1">
            ...
            <div className="grid gap-4 py-2">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditingFaq((f) => (f ? { ...f, question: e.target.value } : f))
                  }
                />
              </div>
              <div className="space-y-1">
