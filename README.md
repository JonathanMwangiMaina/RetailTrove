...
> **Status:** Phases 1–4 complete. Latest: **v0.13.4** — M-Pesa security hardening (P4: receipt encryption + replay protection) and M-Pesa pipeline optimizations (P3: lazy STK push, rate limiting, schema validation). Behind it: v0.13.2 (P2: dev experience + vendor integration), v0.13.1 (P1: observability), v0.13.0 (Phase 3 reliability: migration baseline + ledger), v0.12.0 (security hardening, admin inventory delete fix, seed refactor, documentation cleanup), v0.11.0 (product reviews + server-side currency wiring), v0.10.1 (product images migrated to Supabase Storage), v0.10.0 (security remediation from an external pentest...). **248 passing tests.**
